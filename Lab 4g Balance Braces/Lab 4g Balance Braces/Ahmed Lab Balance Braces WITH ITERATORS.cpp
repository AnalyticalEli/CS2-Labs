////Eli Ahmed
////Lab 4d Balance Braces (USING STRING ITERATORS WITH CHAR STACK) 
//
//#include<iostream>
...
//
//
//bool isBalanced(char argChar);   //Using bool because it takes the least amount of memory + I immediately know if it is OR isn't balanced.
//
//
...
//
//bool isBalanced(char argChar) {  //Using bool as it takes the least amount of memory + I immediately know if it is OR isn't balanced.
//
//
//
//	return(argChar == '(' || argChar == '[' || argChar == '{' || argChar == ')' || argChar == ']' || argChar == '}');
//
//
//	//The ORs (||) will give us a false for boolean if any balance here is NOT met. IF NOT met that will immediately trigger
//	//the function below in the main. Also order of braces don't matter in return ABOVE so code is more extensible.
//
//
//
//}
...
//
//
//bool stringChecker(string argStr) {   // C-type Function that iterators through each char if there
//
//
...
//
//
//	//Auto helps us alot here, starting iterator from beginning
//
//	for (auto itrString = argStr.begin(); itrString != argStr.end(); itrString++) {  //Starting at beginning and iterating through stack
//
//		char current = *itrString;    //Our current string ptr. 
//
