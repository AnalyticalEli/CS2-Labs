...
public: 
	...
	Fighter(string cName, int cLevel, int cStrength, int cDexterity,  int cConstitution, int cIntelligence, int cWisdom, 
...
public:
	...
	Mage (string cName, int cLevel, int cStrength, int cDexterity, int cConstitution, int cIntelligence, int cWisdom,
...
public:
	...
	Cleric(string cName, int cLevel, int cStrength, int cDexterity, int cConstitution, int cIntelligence, int cWisdom,
...
public:
	...
	Thief(string cName, int cLevel, int cStrength, int cDexterity, int cConstitution, int cIntelligence, int cWisdom,
...
//C-type Function Prototype for the Stats (DICE) 
...
int main(void) {
	...
	vector<PlayerChar*> myCharactersList;  //starting the vector with the Playerchar ptr


	//Objective 8 objects (2 from each class)

	//One has default & one has custom
	myCharactersList.push_back(new Fighter("Dragon Borne", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat()));
	myCharactersList.push_back(new Fighter("Juan", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), "Beast Slayer"));


	myCharactersList.push_back(new Mage("Gandalf", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat()));
	myCharactersList.push_back(new Mage("Jones", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), "Bestowing Knowledge"));


	myCharactersList.push_back(new Cleric("Elijah", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat()));
	myCharactersList.push_back(new Cleric("Ezekiel", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), "Let there be A+"));

	myCharactersList.push_back(new Thief("Clepto Maniac", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), "Tax Evasion"));
	myCharactersList.push_back(new Thief("Social Media Goon", generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat(), generateStat()));
	...
	for (auto character : myCharactersList) {  //Found this out with Godot + C++ extension that you can use auto to do polymorphism across ALL elements
											   // in vector irrespective of data type which is epic. (Because you loop without having to specify) 

		character->printCharInfo();   //Calls the pritCharInfo to each & because I have this as a stand in for rewriting it for each it is extra handy
		cout << endl;
	}

	// Memory management (AKA Deleting and deallocating Memory AFTER use) 
	for (auto character : myCharactersList) { //Goes over each new character in the list and finds them all, by figuring out every individual element in the list (myChar) as Ptrs to PlayerChar 
		delete character;   //This allows me to delete 

		character = nullptr;  //And NEVER forget to the PTR itself to nullptr after deletion ;)
	}
...
int generateStat(void) {
	...
	return rollDie() + rollDie() + rollDie();    //3 dice rolls gives you 3 to 18 for stats 
