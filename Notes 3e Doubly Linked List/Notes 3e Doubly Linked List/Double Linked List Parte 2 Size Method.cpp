...
// An abstraction of an array. (WITH Size Method A little More Efficient) 
// 02/21/2024
...
int main(void) {
    ...
    cout << "After adding node ab, we have " << myList.sizeOfList() << "nodes in the list. \n" << endl;
...
int ourLinkedList::sizeOfList(void) {

    Node* temp = getHead();
      int counter = 0;

      if (temp == nullptr) { //This accomdates empty List

          cout << "This was an empty list! There's nothing in it, dumbass. " << endl;
      ...
      //BELOW IS THE SAME WAY OF DOING ABOVE just ONE line of code more.

      //while (temp->getNextPtr() != nullptr) {   //When the next ptr is not the end.
       
      //counter++;  //Accumulator each time its ran
