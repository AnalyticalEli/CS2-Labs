...
int main(void) {
   ...
   //Our for loop dynmaic Generator for the 300 PTR Objects in the employeeList, is the try
	...
	for (int i = 0; i < 300; i++) {   //Accumulates until right before 300
		...
		//Employee PTR that creates new employee object
		Employee* newEmployee = new Employee(firstName, lastName, age, ssn, employeeID, empSalary);



		try {  //Try blocks checks if there any supsicious things with salary, IF NOT it will INSERT ALL VALUES into the list.


			//This uses a ptr to check salary after getting new ptr
			//for any anomolies
			newEmployee->checkSalary(empSalary);


			//Then if no anomolies then inputs new value into list.

			employeeList.push_back(newEmployee);

			newEmployee->displayEmployee();  //Displays the actual info on each "element" within the employeeList. (EVERY EMPLOYEE)b
			...
			cout << "\nError for Employee Number:(" << i << ") \n" << e.what() << endl;   //The i here tells us which one, and the e.what() tells us what's WRONG.
		...
		//Prints Employee Info: Still inside the try block    (Putting for Loop inside for loop above is inefficent in Big O notation, quicker to set it outside)
	for (auto emp : employeeList) {  //Emp Ptr Object within employeeList (auto

		delete emp;  //Deleting "new" (Delete for every new)

		emp = nullptr;  //After every ptr, because we DON'T KNOW how long list is we set each to nullptr after its used.
...
void Person::displayPerson(void) {
	...
	cout << "\nClassified US Citizen Info, for Jefe's eyes only!!" << endl;   //Funny message for declassified
...
void Uscitizen::displayUsInfo(void) {   //Set precision 0 to ensure no decimals 

	cout << "Social Security Number: " << fixed << setprecision(0) << getSSN() << endl;
...
void Employee::checkSalary(double salaryC) {
	...
	if (salaryC >= 0 && salaryC <= 10000) {   //If between 0 and 10000, gives the "truth"
		throw exception("Tax Fraud or Offshore Account");
	}
	else if (salaryC >= 10000 && salaryC < 30000)    //If salary greater than 10000 but less than $30,000 (minimum salary to work there), underpaid.
