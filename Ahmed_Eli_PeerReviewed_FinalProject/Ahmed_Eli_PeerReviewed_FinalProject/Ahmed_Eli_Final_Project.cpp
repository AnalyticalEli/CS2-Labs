...
int main(void) {
    ...
    PlayerChar* Protagonist = new PlayerChar("Steel ja Art", 14, "Thief", "Swindle", "", gen, gen, gen, gen, gen, gen);  //Our Thief
    ...
    PlayerChar* antagonistLackey3 = new PlayerChar("Mediocre Research Advisor", 13, "Thief", "Pelifered Published Papers", "", gen, gen, gen, gen, gen, gen);  //This is cheeky fun, no offense intended for Job Applications. 
    ...
    PlayerChar* dlcChar = new PlayerChar("Judas Jealous", 22, "Thief", "Baldur's Betrayal", "Soul Stealing ((1%) of insta-death)", gen, gen, gen, gen, gen, gen);
    ...
    //First Sorting Jobs (Thief, Paladin, Cleric, Mage). 
    ...
    for (int i = 0; i < jobBucketSize; i++) { // names[i].at(j), cout everything in it. 12 For the Characters
        //for (int j = 0; j < jobBucketSize; j++) {  //4 Bucket Values (Jobs: Thief, Cleric, Mute
